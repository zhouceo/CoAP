...
#define HUAWEIIP "124.70.30.197"
#define HUAWEIPORT 5683
...
int main(int argc, char *argv[])
{
    ...
    ser_addr.sin_addr.s_addr = inet_addr(HUAWEIIP);
    ser_addr.sin_port = htons(HUAWEIPORT);
    ret = huawei_iot_CoAP_Auth(epname, strlen(epname), COAP_MESSAGE_CODE_POST);
    if (!ret)
    {
        res = UDP_Send(CoAPpack, CoAPpack_len);
        if (res == CoAPpack_len)
        {
            huawei_ACK();
        }
...
uint16_t UDP_Receive(uint8_t *data, uint16_t maxLen)
{
    ...
    cnt = recvfrom(sockfd, data, maxLen, 0, (struct sockaddr *)&cli_addr, &len);
    if (cnt == -1)
    {
        return 0;
    }
...
uint8_t UDP_Send(uint8_t *data, uint16_t len)
{
    ...
    printf("\r\n");
    res = sendto(sockfd, data, len, 0, (struct sockaddr *)&ser_addr, sizeof(ser_addr));
    if (res == -1)
    {

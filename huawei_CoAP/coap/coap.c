...
int huawei_iot_CoAP_Auth(char *ep, uint8_t epLen, unsigned char Code)
{
    ...
    memset(CoAPpack, 0, 1024);
    unsigned char CoAP_head[4] = {0};
    unsigned char CoAP_token[2] = {0};
    unsigned char Path_Option[4] = {0};
    unsigned char Content_Option[2] = {0};
    unsigned char Query_Option[30] = {0};
    int Query_Option_Len = 0;
    CoAP_head[0] = 0x42;
    CoAP_head[1] = Code;
    CoAP_head[2] = Message_ID / 256;
    CoAP_head[3] = Message_ID % 256;
    ...
    // path
    Path_Option[0] = 0xB1;
    Path_Option[1] = 0x74; // t
    Path_Option[2] = 0x01;
    Path_Option[3] = 0x72; // r
    // Content-Format
    Content_Option[0] = 0x11;
    Content_Option[1] = 0x2a;
    // Query
    Query_Option[0] = 0x3D;
    Query_Option[1] = epLen + 3 - 13;
    Query_Option[2] = 0x65;
    Query_Option[3] = 0x70;
    Query_Option[4] = 0x3D; // ep=
    for (int m = 0, j = 5; m < epLen; m++, j++)
    {
        Query_Option[j] = ep[m];
    }
    Query_Option_Len = epLen + 5;

    memset(CoAPpack, 0, 1024);
    memcpy(&CoAPpack[0], CoAP_head, 4);
    memcpy(&CoAPpack[4], CoAP_token, 2);
    memcpy(&CoAPpack[4 + 2], Path_Option, 4);
    memcpy(&CoAPpack[4 + 2 + 4], Content_Option, 2);
    memcpy(&CoAPpack[4 + 2 + 4 + 2], Query_Option, Query_Option_Len);

    CoAPpack_len = 4 + 2 + 4 + 2 + Query_Option_Len;
...
int huawei_ACK(void)
{
    uint8_t TokenNotify[16] = {0};
    int res = 0, ret = 0, TklNotify = 0;
    res = HWReceiveData(RxBuff, RXBUFF_SIZE, 2000);
    if (RxBuff[0] != 0x62 || RxBuff[1] != 0x44)
    {
        return -1;
    }
    memset(RxBuff, 0, sizeof(RxBuff));
    res = HWReceiveData(RxBuff, RXBUFF_SIZE, 30000);
    ret = (RxBuff[0] & 0x0f) + 4;
    if (((RxBuff[0] & 0xf0) != 0x40) || (RxBuff[1] != 0x01) || (RxBuff[ret] != 0x60))
    {
        return -1;
    }
    TklNotify = RxBuff[0] & 0x0f;
    for (int i = 0; i < TklNotify; i++)
    {
        TokenNotify[i] = RxBuff[4 + i];
    }
    RxBuff[0] |= 0x20;
    RxBuff[1] = 0x44;
    res = UDP_Send(RxBuff, 5 + TklNotify);
    memset(RxBuff, 0, sizeof(RxBuff));
    res = HWReceiveData(RxBuff, RXBUFF_SIZE, 3000);
    ret = (RxBuff[0] & 0x0f) + 4 + 3 + 1;
    if (RxBuff[0] & 0xf0 != 0x40 || RxBuff[1] != 0x01 || RxBuff[ret] != 0x34)
    {
        return -1;
    }
    RxBuff[0] |= 0x20;
    RxBuff[1] = 0x84; // 4.04 not found
    ret = (RxBuff[0] & 0x0f) + 4;
    res = UDP_Send(RxBuff, ret);
}
